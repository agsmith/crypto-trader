...
def make_magic(curr_price, last_trans_price, high_price):
  ...
  if(curr_price > (last_trans_price + (last_trans_price * SELL_PTG))):
    #transslack.notify(text=":metal: Selling LTC at $" + str(curr_price))
    # sellLTC(curr_price)
    return curr_price;
  elif(curr_price < (high_price - (high_price * BUY_PTG))):
    #transslack.notify(text=":ltc: Buying LTC at $" + str(curr_price))
    # buyLTC(curr_price)
    return curr_price;
  else:
...
def on_message(socket, msg):
    ...
    if u'price' in j:
      ...
      socket.last_trans_price = make_magic(socket.curr_price, socket.last_trans_price, socket.high_price)
      if(float(j[u'price']) > socket.high_price):
        socket.high_price = float(j[u'price'])
        hpslack.notify(text=":ltc: LTC $" + str(socket.high_price) + " buy at $" + str(socket.high_price - (socket.high_price*BUY_PTG)) + " sell at " + str((socket.last_trans_price + (socket.last_trans_price * SELL_PTG))))

def buyLTC( current_price ):
  "buy USD worth of LTC"
  s = str(truncate(ORDER_SIZE/current_price, 3))
  buy = auth_client.buy(price=str(ORDER_SIZE), size=s,product_id='LTC-USD')
  return;

def sellLTC( current_price ):
  "sell 100USD worth of LTC"
  s = str(truncate(ORDER_SIZE/current_price, 3))
  sell = auth_client.sell(price=str(ORDER_SIZE), size=s, product_id='LTC-USD')
  # print json.dumps(sell, indent=4, sort_keys=True)
  return;

def go():
    """Main function."""
    hpslack.notify(text=":1up: ltc-trader restarted")
